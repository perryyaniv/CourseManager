...
export default function Sidebar({ open, onClose }: Props) {
  ...
  const navItems = [
    { to: '/', label: t('nav.dashboard'), icon: HomeIcon, show: true },
    { to: '/courses', label: t('nav.courses'), icon: CoursesIcon, show: true },
    { to: '/courses/new', label: t('nav.addCourse'), icon: AddIcon, show: isCoordinator },
    { to: '/audit-log', label: t('nav.auditLog'), icon: LogIcon, show: isAdmin },
    { to: '/users', label: t('nav.userManagement'), icon: UsersIcon, show: isAdmin },
    { to: '/settings', label: t('nav.settings'), icon: SettingsIcon, show: isAdmin },
  ];
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 right-0 z-40 h-full w-64 bg-white shadow-xl transform transition-transform duration-200 flex flex-col ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-800">CourseManager</span>
          </div>
        ...
        <div className="px-3 py-4 border-t border-gray-100">
          <div className="px-3 py-2 text-xs text-gray-400 mb-1">{user?.username} · {user?.role}</div>
          <button
